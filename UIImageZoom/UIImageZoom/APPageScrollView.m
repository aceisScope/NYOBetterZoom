...
{
    if ([scrollView isKindOfClass:[NYOBetterZoomUIScrollView class]]) 
    {
        return;
    }
    
    if (scrollView.contentOffset.x <= currentPageNum*scrollView.frame.size.width) 
    {
    ...
    if (pageNum == currentPageNum + 1) {
        [self _nextPage];
        [self willDisplayPageAtIndex:pageNum withDirection:self.scrollDirection];
...
- (void) handleTapEvent:(UIGestureRecognizer *)recognizer 
{
    ...
    NSLog(@"single tap on PageScrollView");
...
- (void) willDisplayPageAtIndex:(int)index  withDirection:(int)direction{
    // do nothing
    // to show the label if needed;
    if ([self.delegate respondsToSelector:@selector(pageScrollView:didShowPageAtIndex:)]) {
        [self.delegate pageScrollView:self didShowPageAtIndex:index];
